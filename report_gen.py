...
parser.add_argument('-o', '--out', help='name of output .docx-file')
parser.add_argument('-p', '--picture', help='name of the drawing of the molecular structure')
...
if args.user:
    ...
    replacements['USER'] = input('Please input name of the synthetic chemist: ')
if not args.out:
    args.out = input('Please enter the filename of the report to be generated: ')
        ...
        for i in inline:
            if key in i.text:
                if key == 'SPGRP':
                    ...
                    new_dom = transform(etree.fromstring(spgrp.iucrNumberToMathml(replacements[key])))
